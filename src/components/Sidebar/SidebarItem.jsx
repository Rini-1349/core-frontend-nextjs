 ...
 * @returns {JSX.Element}
 */
const SidebarItem = ({ item, pageName, setPageName, session }) => {
  const pathname = getEnglishSlug(usePathname());
  const isActive = item.route === pathname;

  // Vérifie si au moins un enfant correspond à la route active
  const isChildActive = item.children?.some((child) => matchPathname(pathname, child.isActiveRoutes));
  ...
  return (
    <>
      <li>
        <Link href={item.route} onClick={handleClick} className={`${isActive || isChildActive ? "bg-graydark" : ""} group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark`}>
          {item.icon && <FontAwesomeIcon icon={item.icon} />}
          {item.label}
          {item.children && (
            <svg className={`absolute right-4 top-1/2 -translate-y-1/2 fill-current ${pageName === item.label.toLowerCase() && "rotate-180"}`} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {item.children && (
          <div className={`translate transform overflow-hidden ${pageName !== item.label.toLowerCase() && "hidden"}`}>
            <SidebarDropdown item={item.children} session={session} />
          </div>
        )}
      </li>
    </>
