...
const SidebarItem = ({ item, pageName, setPageName }) => {
  ...
  useEffect(() => {
    setIsClient(true); // Détecte si l'on est côté client
    setPathname(window.location.pathname); // Récupère pathname sur le client
  }, []);
  ...
  if (!isClient) {
    return null; // On retourne rien tant qu'on est côté serveur
  }
